...
def load_data():
    ...
    df_pred_sent    = pd.read_csv("hasil_prediksi_lstm_sentimen.csv")
    df_pred_tech    = pd.read_csv("hasil_prediksi_lstm_teknikal.csv")
    df_eval_sent    = pd.read_csv("evaluasi_lstm_sentimen.csv")
    df_eval_tech    = pd.read_csv("evaluasi_lstm_teknikal.csv")
    return df_raw, df_pred_sent, df_pred_tech, df_eval_sent, df_eval_tech

# ------------------------------------------------------------------
# Catatan migrasi nama file CSV:
#   LAMA  → BARU
#   hasil_prediksi_xgb_sentimen_t1.csv  → hasil_prediksi_lstm_sentimen.csv
#   hasil_prediksi_xgb_teknikal_t1.csv  → hasil_prediksi_lstm_teknikal.csv
#   evaluasi_xgb_sentimen_t1.csv        → evaluasi_lstm_sentimen.csv
#   evaluasi_xgb_teknikal_t1.csv        → evaluasi_lstm_teknikal.csv
#
# Kolom wajib di setiap file prediksi: 'Actual', 'Prediction', 'Date'(opsional)
# Kolom wajib di file evaluasi       : 'RMSE', 'MAE', 'R2'  (minimal RMSE)
# ------------------------------------------------------------------
...
rmse_sent   = df_eval_sent['RMSE'].iloc[0] if 'RMSE' in df_eval_sent.columns else 0
mae_sent    = df_eval_sent['MAE'].iloc[0]  if 'MAE'  in df_eval_sent.columns else 0
r2_sent     = df_eval_sent['R2'].iloc[0]   if 'R2'   in df_eval_sent.columns else 0

rmse_tech   = df_eval_tech['RMSE'].iloc[0] if 'RMSE' in df_eval_tech.columns else 0
mae_tech    = df_eval_tech['MAE'].iloc[0]  if 'MAE'  in df_eval_tech.columns else 0
r2_tech     = df_eval_tech['R2'].iloc[0]   if 'R2'   in df_eval_tech.columns else 0
...
# ==============================================================================
    ...
    # Susun dataframe plot
    n = min(len(df_pred_sent), len(df_pred_tech))
    actual_col = 'Actual' if 'Actual' in df_pred_sent.columns else df_pred_sent.columns[0]
    pred_s_col = 'Prediction' if 'Prediction' in df_pred_sent.columns else df_pred_sent.columns[-1]
    pred_t_col = 'Prediction' if 'Prediction' in df_pred_tech.columns else df_pred_tech.columns[-1]
...
# ==============================================================================
    ...
    # ── Kartu evaluasi
    metrics_list = []
    for col_name in ['RMSE', 'MAE', 'R2', 'MAPE']:
        vs = df_eval_sent[col_name].iloc[0] if col_name in df_eval_sent.columns else None
        vt = df_eval_tech[col_name].iloc[0] if col_name in df_eval_tech.columns else None
        if vs is not None or vt is not None:
            metrics_list.append((col_name, vs, vt))
