...
# ==============================================================================
st.set_page_config(
    page_title="XGBoost StockVision · Prediksi Volatilitas Saham",
    page_icon="⚡",
    layout="wide",
...
# ==============================================================================
# NAMA KOLOM TETAP SESUAI CSV ASLI
# ==============================================================================
# hasil_prediksi_xgb_sentimen_t1.csv :
#   Tanggal | Kode_Emiten | Volatilitas_Aktual | Volatilitas_Pred_XGB_Sentimen_t1 | Error_Abs
# hasil_prediksi_xgb_teknikal_t1.csv :
#   Tanggal | Kode_Emiten | Volatilitas_Aktual | Volatilitas_Pred_XGB_Teknikal_t1 | Error_Abs

COL_AKTUAL   = 'Volatilitas_Aktual'
COL_PRED_S   = 'Volatilitas_Pred_XGB_Sentimen_t1'
COL_PRED_T   = 'Volatilitas_Pred_XGB_Teknikal_t1'
COL_TANGGAL  = 'Tanggal'
COL_EMITEN   = 'Kode_Emiten'
COL_ERR      = 'Error_Abs'

# ==============================================================================
# PREPARASI METRIK
# ==============================================================================
# Volatilitas terakhir dari data prediksi
vol_terakhir = df_pred_sent[COL_AKTUAL].iloc[-1] if COL_AKTUAL in df_pred_sent.columns else 0
vol_delta    = ((df_pred_sent[COL_AKTUAL].iloc[-1] - df_pred_sent[COL_AKTUAL].iloc[-2])
                / df_pred_sent[COL_AKTUAL].iloc[-2] * 100
                if COL_AKTUAL in df_pred_sent.columns and len(df_pred_sent) > 1 else 0)

# Harga close dari df_raw jika ada
...
# Hitung RMSE langsung dari data prediksi jika file evaluasi tidak tersedia
if rmse_sent == 0 and COL_AKTUAL in df_pred_sent.columns and COL_PRED_S in df_pred_sent.columns:
    rmse_sent = float(np.sqrt(np.mean((df_pred_sent[COL_AKTUAL] - df_pred_sent[COL_PRED_S])**2)))
if rmse_tech == 0 and COL_AKTUAL in df_pred_tech.columns and COL_PRED_T in df_pred_tech.columns:
    rmse_tech = float(np.sqrt(np.mean((df_pred_tech[COL_AKTUAL] - df_pred_tech[COL_PRED_T])**2)))
if mae_sent == 0 and COL_ERR in df_pred_sent.columns:
    mae_sent = float(df_pred_sent[COL_ERR].mean())
if mae_tech == 0 and COL_ERR in df_pred_tech.columns:
    mae_tech = float(df_pred_tech[COL_ERR].mean())
if r2_sent == 0 and COL_AKTUAL in df_pred_sent.columns and COL_PRED_S in df_pred_sent.columns:
    ss_res = np.sum((df_pred_sent[COL_AKTUAL] - df_pred_sent[COL_PRED_S])**2)
    ss_tot = np.sum((df_pred_sent[COL_AKTUAL] - df_pred_sent[COL_AKTUAL].mean())**2)
    r2_sent = float(1 - ss_res/ss_tot) if ss_tot != 0 else 0
if r2_tech == 0 and COL_AKTUAL in df_pred_tech.columns and COL_PRED_T in df_pred_tech.columns:
    ss_res = np.sum((df_pred_tech[COL_AKTUAL] - df_pred_tech[COL_PRED_T])**2)
    ss_tot = np.sum((df_pred_tech[COL_AKTUAL] - df_pred_tech[COL_AKTUAL].mean())**2)
    r2_tech = float(1 - ss_res/ss_tot) if ss_tot != 0 else 0

# Daftar emiten unik
emiten_list = sorted(df_pred_sent[COL_EMITEN].unique().tolist()) if COL_EMITEN in df_pred_sent.columns else ['ADRO']

model_terbaik = "XGBoost + Sentimen 📰" if rmse_sent < rmse_tech else "XGBoost Teknikal 💻"
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:16px 0 10px'>
      ...
      <div style='font-size:18px;font-weight:800;letter-spacing:-.5px'>XGBoost StockVision</div>
      <div style='font-size:10px;color:#5A6A85;letter-spacing:2px;margin-top:4px;font-family:Space Mono,monospace'>MACHINE LEARNING · SAHAM ENERGI</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
...
if menu == "🏠 Dashboard":
    ...
    <div style='padding:10px 0 4px'>
      ...
      <p style='font-size:14px;color:#5A6A85;margin:6px 0 0;font-family:Space Mono,monospace'>
        XGBoost · Saham Sektor Energi LQ45 · Periode Pandemi COVID-19
      </p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col_filter:
        emiten_dipilih = st.selectbox("Filter Emiten:", ["Semua"] + emiten_list, key="dash_emiten")

    df_s_filter = df_pred_sent if emiten_dipilih == "Semua" else df_pred_sent[df_pred_sent[COL_EMITEN] == emiten_dipilih]
    df_t_filter = df_pred_tech if emiten_dipilih == "Semua" else df_pred_tech[df_pred_tech[COL_EMITEN] == emiten_dipilih]

    vol_now   = df_s_filter[COL_AKTUAL].iloc[-1] if len(df_s_filter) > 0 else 0
    vol_prev  = df_s_filter[COL_AKTUAL].iloc[-2] if len(df_s_filter) > 1 else vol_now
    vol_delta_now = (vol_now - vol_prev) / vol_prev * 100 if vol_prev != 0 else 0
    mae_now   = df_s_filter[COL_ERR].mean() if COL_ERR in df_s_filter.columns else mae_sent
    mae_t_now = df_t_filter[COL_ERR].mean() if COL_ERR in df_t_filter.columns else mae_tech
    ...
    with c1:
        ...
        <div class="kpi-card accent-blue">
          ...
          <div class="kpi-label">Volatilitas Terakhir</div>
          <div class="kpi-value">{vol_now:.5f}</div>
          <div class="kpi-delta {'down' if vol_delta_now >= 0 else 'up'}">
            {'▲' if vol_delta_now >= 0 else '▼'} {abs(vol_delta_now):.2f}%
          </div>
    ...
    with c2:
        ...
        <div class="kpi-card accent-green">
          ...
          <div class="kpi-delta neu">XGB TERBAIK</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c5:
        n_emiten = len(emiten_list)
        st.markdown(f"""
        <div class="kpi-card accent-yellow">
          <div class="kpi-icon">🏢</div>
          <div class="kpi-label">Jumlah Emiten</div>
          <div class="kpi-value">{n_emiten}</div>
          <div class="kpi-delta neu">Sektor Energi</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div class="insight-box green">
      ...
      Berdasarkan nilai galat terkecil dari dua pendekatan XGBoost (Teknikal vs Sentimen), model dengan performa optimal adalah <b>{model_terbaik}</b>.
    </div>
    """, unsafe_allow_html=True)

    # ── Mini Volatilitas Chart
    st.markdown("""<div class="section-header">
      <div class="section-dot" style="background:#00D4FF"></div>
      <h3>Volatilitas Return Historis</h3>
    </div>""", unsafe_allow_html=True)

    if COL_AKTUAL in df_s_filter.columns:
        vol_series = df_s_filter[COL_AKTUAL].reset_index(drop=True)
        x_dash = (pd.to_datetime(df_s_filter[COL_TANGGAL]).dt.strftime('%d %b %Y').tolist()
                  if COL_TANGGAL in df_s_filter.columns else list(range(len(vol_series))))
        fig_price = go.Figure()
        fig_price.add_trace(go.Scatter(
            x=x_dash, y=vol_series, mode='lines',
            line=dict(color='#00D4FF', width=2),
            fill='tozeroy', fillcolor='rgba(0,212,255,0.06)',
            name='Volatilitas Aktual'
        ))
        if COL_PRED_S in df_s_filter.columns:
            fig_price.add_trace(go.Scatter(
                x=x_dash, y=df_s_filter[COL_PRED_S].reset_index(drop=True),
                mode='lines', line=dict(color='#7B61FF', width=1.5, dash='dash'),
                name='Pred Sentimen'
            ))
        if COL_PRED_T in df_t_filter.columns:
            vol_t = df_t_filter[COL_PRED_T].reset_index(drop=True)
            fig_price.add_trace(go.Scatter(
                x=x_dash[:len(vol_t)], y=vol_t,
                mode='lines', line=dict(color='#FF6B35', width=1.5, dash='dot'),
                name='Pred Teknikal'
            ))
        fig_price.update_layout(
            ...
            xaxis=dict(showgrid=False, color='#5A6A85',
                       tickmode='auto', nticks=10, tickangle=-30),
            yaxis=dict(gridcolor='#1E2A45', color='#5A6A85',
                       title='Volatilitas Return'),
            legend=dict(font=dict(size=11), bgcolor='rgba(15,21,38,0.8)')
        )
    ...
    with col_a:
        ...
        <div class="insight-box purple">
          <b>🔬 Metode XGBoost (Extreme Gradient Boosting)</b><br><br>
          Penelitian ini menggunakan <b>XGBoost</b> — algoritma ensemble berbasis pohon keputusan 
          yang sangat efektif untuk data tabular dan time-series keuangan. 
          XGBoost mampu menangkap pola non-linear dari indikator teknikal dan sentimen 
          untuk menghasilkan prediksi volatilitas return yang akurat.
        </div>
        """, unsafe_allow_html=True)
    with col_b:
        st.markdown("""
        <div class="insight-box orange">
          ...
          Bollinger Bands, SMA, dan Volume sebagai fitur input XGBoost.<br><br>
          <b>2. Analisis Sentimen:</b> Mengekstrak skor sentimen dari berita/media 
          menggunakan NLP, dikombinasikan dengan data historis sebagai fitur XGBoost.
        </div>
        """, unsafe_allow_html=True)


# ==============================================================================
# ─── HALAMAN 2: PREDIKSI XGBoost ─────────────────────────────────────────────
# ==============================================================================
elif menu == "📈 Prediksi LSTM":

    st.markdown("""
    <h2 style='font-size:22px;font-weight:800;color:#E8EDF5;margin-bottom:4px'>
      📈 Visualisasi Prediksi Model XGBoost
    </h2>
    <p style='color:#5A6A85;font-size:13px;font-family:Space Mono,monospace;margin-bottom:16px'>
      Volatilitas Aktual vs Prediksi — Model Sentimen &amp; Model Teknikal
    </p>
    """, unsafe_allow_html=True)

    col_f1, col_f2 = st.columns([2, 3])
    with col_f1:
        emiten_pred = st.selectbox("Filter Emiten:", ["Semua"] + emiten_list, key="pred_emiten")
    with col_f2:
        model_pilih = st.selectbox(
            "Tampilkan model:",
            ["Keduanya (Perbandingan)", "XGBoost + Sentimen", "XGBoost Teknikal"],
        )

    # Filter per emiten
    dfs = df_pred_sent if emiten_pred == "Semua" else df_pred_sent[df_pred_sent[COL_EMITEN] == emiten_pred]
    dft = df_pred_tech if emiten_pred == "Semua" else df_pred_tech[df_pred_tech[COL_EMITEN] == emiten_pred]
    dfs = dfs.reset_index(drop=True)
    dft = dft.reset_index(drop=True)
    n   = min(len(dfs), len(dft))

    # Gunakan kolom eksak dari CSV
    aktual    = dfs[COL_AKTUAL].values[:n]
    pred_sent = dfs[COL_PRED_S].values[:n]
    pred_tech = dft[COL_PRED_T].values[:n]

    # Gunakan Tanggal sebagai x-axis jika tersedia
    if COL_TANGGAL in dfs.columns:
        x_idx = pd.to_datetime(dfs[COL_TANGGAL]).dt.strftime('%d %b %Y').values[:n].tolist()
        x_title = 'Tanggal'
    else:
        x_idx   = list(range(n))
        x_title = 'Index Pengujian'
    ...
    if model_pilih in ["Keduanya (Perbandingan)", "XGBoost + Sentimen"]:
        fig2.add_trace(go.Scatter(
    ...
    if model_pilih in ["Keduanya (Perbandingan)", "XGBoost Teknikal"]:
        fig2.add_trace(go.Scatter(
    ...
    fig2.update_layout(
        ...
        xaxis=dict(title=x_title, gridcolor='#1E2A45', color='#5A6A85'),
        yaxis=dict(title='Volatilitas Return', gridcolor='#1E2A45', color='#5A6A85'),
        legend=dict(
...
<div style='display:flex;justify-content:space-between;align-items:center;
  ...
  <div style='font-size:12px;color:#5A6A85;font-family:Space Mono,monospace'>
    ⚡ XGBoost StockVision · Pemodelan Return Saham Sektor Energi LQ45
  </div>
  <div style='font-size:12px;color:#5A6A85;font-family:Space Mono,monospace'>
